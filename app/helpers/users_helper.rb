module UsersHelper
  ...
  def user_followers
    followers_array = @user.inverse_followings.map do |following|
  ...
  def avatar_present?(user)
    if user.avatar.url.present?
      ...
      content_tag(:i, class: 'fa fa-user-circle user-icon') { ; }
    end
  end
  ...
  def render_user_details(user)
    content_tag :div, class: 'who-to-follow-details' do
      user_fullname = content_tag :p do
        content_tag(:strong, user.fullname.to_s)
      end
  ...
  def who_to_follow
    content_tag :ul do
      User.all.ordered_by_most_recent[0..6].each do |user|
