module CoverphotoZoneHelper
  ...
  def render_userpage_reviews_link
    link_to user_reviews_path(@user, @reviews) do
      reviews_count = content_tag(:h4, @user.inverse_reviews.count.to_s, class: 'reviews-count')
  ...
  def render_userpage_followings
    content_tag :div do
      followings_count = content_tag(:h4, @user.followings.count.to_s)
  ...
  def render_userpage_followers
    content_tag :div do
      followers_count = content_tag(:h4, @user.inverse_followings.count.to_s)
